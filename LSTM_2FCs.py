...
class Net(nn.Module):
	...
	def forward(self, x):
		...
		intermediate_outputs = torch.zeros(self.num_tasks, self.batch_size, 64).to(DEVICE)
		for i in range(self.num_tasks):
			intermediate_outputs[i] = self.fc3[i](out)
		
		outputs = torch.zeros(self.num_tasks, self.batch_size, self.output_size).to(DEVICE)
		for i in range(self.num_tasks):
			outputs[i] = self.fc4[i](intermediate_outputs[i])
		return outputs
