...
class Net(nn.Module):
	def __init__(self, input_size, hidden_size, output_size, num_tasks, batch_size):
		...
		self.fc2 = nn.ModuleList([nn.Linear(256, 128) for i in range(num_tasks)])
		self.fc3 = nn.ModuleList([nn.Linear(128, 64) for i in range(num_tasks)])
	...
	def forward(self, x):
		...
		intermediate_outputs1 = torch.zeros(self.num_tasks, self.batch_size, 128).to(DEVICE)
		for i in range(self.num_tasks):
			intermediate_outputs1[i] = self.fc2[i](out)
		
		intermediate_outputs2 = torch.zeros(self.num_tasks, self.batch_size, 64).to(DEVICE)
		for i in range(self.num_tasks):
			intermediate_outputs2[i] = self.fc3[i](intermediate_outputs1[i])
		
		outputs = torch.zeros(self.num_tasks, self.batch_size, self.output_size).to(DEVICE)
		for i in range(self.num_tasks):
			outputs[i] = self.fc4[i](intermediate_outputs2[i])
		return outputs
